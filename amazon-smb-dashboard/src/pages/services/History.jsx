import "./History.css";import React from 'react';
...
const ShipmentTimeline = () => {
  const shipmentData = [
    ...
    { status: "Delivered", location: "Abu Dhabi, UAE", time: "June 16, 2023 at 10:45 AM", isLast: true, noIcon: true }, // no icon for "Delivered"
  ];

  return (
    <div className="shipment-timeline">
     <h2 style={{ fontSize: '40px', textAlign: 'left', color: 'black',paddingLeft:'15px',marginBottom:'10px' }}>Shipment Timeline</h2>
        ...
        {shipmentData.map((item, index) => (
          ...
          >
            <div className={`timeline-icon ${item.isFirst ? 'location-icon' : 'check-icon'}`}>
              {item.isLast && item.noIcon ? null : item.isFirst ? <FaMapMarkerAlt /> : <FaCheckCircle />}
            </div>
